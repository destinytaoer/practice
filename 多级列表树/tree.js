;(function ($, window, document) {
   ...
   */
	...
	/**
   * @description 初始化 nodes 节点的数据
   * 增加 nodeId、parentId、selectable、state（checked、disabled、expanded、selected）
   */
   ...
   */
    ...
    var node = this.nodes.filter(function (item, index) {
  ...
  /**
   * @description 设置是否选中状态
   */
	Tree.prototype.toggleSelectedState = function (node, options) {
		if (!node) return;
		this.setSelectedState(node, !this.options.multiSelect || !node.state.selected, options);
	};
  ...
  /**
   * @description 渲染树结构
   * @param [data] 可选的，为渲染时使用的数据，默认为一开始的 this.tree 原始数据，增加参数用于搜索时重新渲染列表，但不改变原始数据列表
   */
   ...
   */
	...
	*/
    ...
    return this.nodes.filter(function (item, index) {
      return item.nodeId == nodeId
    })[0]
	};
	...
	$.fn[pluginName] = function (options, args) {
		...
		this.each(function () {
			...
			if (typeof options === 'string') {
				...
				result = $.data(this, pluginName, new Tree(this, $.extend(true, {}, options)));
			}
		});
