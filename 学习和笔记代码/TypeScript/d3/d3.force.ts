...
var d3GraphUtil = {
  transform2Str: function (transform: { x: number, y: number, k: number }) {
    return 'translate(' + transform.x + ',' + transform.y + ') ' + 'scale(' + transform.k + ')'
  },
  measureText: function (text: string, fontSize: number) {
    if (!text || text.length === 0) return { height: 0, width: 0 };
    var container:any = d3.select('body').append('svg').attr('class', 'dummy');
...
(function ($) {
  ...
  class Force {
    svg: any;
    constructor(public ele: any, public options: Option) {
      this.svg = d3.select(ele);
    }
  }
  function (ele, options: Option) {
